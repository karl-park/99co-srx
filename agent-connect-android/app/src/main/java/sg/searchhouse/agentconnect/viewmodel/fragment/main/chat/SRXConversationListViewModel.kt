...
class SRXConversationListViewModel constructor(application: Application) :
    ...
    fun initialLoadSRXConversations() {
        startIndex = 0
        selectedConversations.clear()
        status.postValue(LOADING)
        loadSRXConversationsFromDB()
        loadSRXConversationsFromServer()
    }

    fun loadMoreSRXConversations() {
    ...
    fun refreshSRXConversations() {
        ...
        loadSRXConversationsFromDB(limit)
        loadSRXConversationsFromServer(limit)
    }

    private fun loadSRXConversationsFromDB(maxResults: Int = ApiConstant.MAX_RESULTS_CONVERSATION_LIST) {
        chatDbRepository.getSRXConversations(
            startIndex,
            maxResults
    ...
    private fun loadSRXConversationsFromServer(maxResults: Int = ApiConstant.MAX_RESULTS_CONVERSATION_LIST) {
        ApiUtil.performRequest(
            applicationContext,
            chatRepository.findSRXSsmConversations(
                startIndex,
                maxResults
            ),
            onSuccess = {
    ...
    private fun insertConversations(
        ...
        chatDbRepository.insertSsmConversations(conversations).performAsyncQueriesLenient {
            ...
            loadSRXConversationsFromDB(maxResults)
        }
    }
