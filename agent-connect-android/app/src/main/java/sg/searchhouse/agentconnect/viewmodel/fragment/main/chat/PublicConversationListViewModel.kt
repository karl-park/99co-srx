...
class PublicConversationListViewModel constructor(application: Application) :
    ...
    fun initialLoadPublicConversations() {
        startIndex = 0
        selectedConversations.clear()
        status.postValue(LOADING)
        loadPublicConversationsFromDB()
        loadPublicConversationsFromServer()
    }

    fun loadMorePublicConversations() {
    ...
    fun refreshPublicConversations() {
        startIndex = 0
        selectedConversations.clear()
        status.postValue(LOADING)
        ...
        loadPublicConversationsFromDB(limit)
        loadPublicConversationsFromServer(limit)
    }

    private fun loadPublicConversationsFromDB(maxResults: Int = ApiConstant.MAX_RESULTS_CONVERSATION_LIST) {
        chatDbRepository.getPublicConversations(
    ...
    private fun loadPublicConversationsFromServer(maxResults: Int = ApiConstant.MAX_RESULTS_CONVERSATION_LIST) {
        ApiUtil.performRequest(
            applicationContext,
            chatRepository.findPublicSsmConversations(startIndex, maxResults),
            onSuccess = {
    ...
    private fun insertConversations(
        ...
        chatDbRepository.insertSsmConversations(conversations).performAsyncQueriesLenient {
            ...
            loadPublicConversationsFromDB(maxResults)
        }
    }
